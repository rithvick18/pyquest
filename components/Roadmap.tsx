            ...
            const node = (
              ...
              >
                <button
                  ...
                  disabled={!isUnlocked}
                  className={`relative w-20 h-20 rounded-full flex items-center justify-center transition-all duration-300 shadow-2xl ${isCompleted
                ...
                {/* Floating Label */}
                <div
                  className={`absolute top-1/2 -translate-y-1/2 w-48 transition-all duration-300 ${isLeft
                    ? 'right-full mr-6 text-right'
                    : 'left-full ml-6 text-left'
                    }`}
                >
                  <div className={`text-sm font-bold leading-tight mb-1 transition-colors ${isNext ? 'text-[var(--text-primary)] scale-105 origin-center' :
